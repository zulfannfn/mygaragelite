...
export default function TransactionForm() {
  ...
  const handleWhatsApp = async () => {
    if (!savedTx) return;
    setActionBusy('wa');
    const r = await receiptService.sendWhatsApp(savedTx);
      ...
      >
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 200 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, marginTop: 8 }}>
            ...
            <Pressable onPress={() => setServicePickerOpen(true)}>
              <Text style={{ color: theme.colors.accent, fontWeight: '600' }}>+ Tambah</Text>
            </Pressable>
          </View>
          {services.length === 0 ? (
            ...
            services.map((s, i) => (
              <Card key={i} style={{ marginBottom: 8 }} padding={12}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  ...
                  <Pressable onPress={() => removeService(i)} style={{ padding: 6 }}>
          ...
          >
            ...
            <Pressable onPress={() => setSparepartPickerOpen(true)}>
              <Text style={{ color: theme.colors.accent, fontWeight: '600' }}>+ Tambah</Text>
            </Pressable>
          </View>
          {parts.length === 0 ? (
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ color: theme.colors.accent, fontSize: 13, marginTop: 2 }}>
                      ...
                      {formatCompactCurrency(p.sell_price * p.quantity)}
                    </Text>
                  </View>
                  <Pressable
                  ...
                  <Text style={{ color: theme.colors.text, minWidth: 20, textAlign: 'center' }}>
                    {p.quantity}
                  </Text>
                  <Pressable
                    onPress={() => updatePartQty(i, 1)}
                  ...
                  <Pressable onPress={() => removePart(i)} style={{ padding: 6 }}>
                    <Ionicons name="trash" size={18} color={theme.colors.danger} />
                  </Pressable>
                </View>
              </Card>
        ...
        <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
          ...
          <ScrollView contentContainerStyle={{ padding: 16 }}>
            ...
            {SERVICE_PRESETS.map((s, i) => (
              <Card key={i} style={{ marginBottom: 8 }} onPress={() => addPresetService(s)}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
      ...
      >
        <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <Card
                onPress={() => addSparepart(item)}
                style={{ marginBottom: 8, opacity: item.stock <= 0 ? 0.5 : 1 }}
              >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: theme.colors.text, fontWeight: '600' }}>
                    ...
                    <View style={{ flexDirection: 'row', gap: 6, marginTop: 4 }}>
                      <Badge
                        ...
                        variant={item.stock <= item.min_stock ? 'danger' : 'success'}
                      />
                    </View>
                  </View>
